...
export const useGarage = () => {
  ...
  return {
    ...
    getVehiclesByCustomer: (customerId) =>
      (store.vehicles || []).filter((v) => v.customerId === customerId),
    createJobCard: async (jobCard) => useGarageStore.getState().addJobCard(jobCard),
    createInvoice: async (invoice) => useGarageStore.getState().addInvoice(invoice),
    ...
    updateSparePartStock: async (partId, quantity) => {
      ...
      if (part) {
        await useGarageStore
          .getState()
          .updateSparePart(partId, { stock: Math.max(0, part.stock - quantity) });
      }
    },
    ...
    importData: async (jsonString) => {
      ...
      if (data.customers)
        for (const customer of data.customers)
          await useGarageStore.getState().addCustomer(customer);
      if (data.vehicles)
        for (const vehicle of data.vehicles) await useGarageStore.getState().addVehicle(vehicle);
      if (data.jobCards)
        for (const jobCard of data.jobCards) await useGarageStore.getState().addJobCard(jobCard);
      if (data.appointments)
        for (const appointment of data.appointments)
          await useGarageStore.getState().addAppointment(appointment);
      if (data.mechanics)
        for (const mechanic of data.mechanics)
          await useGarageStore.getState().addMechanic(mechanic);
      if (data.spareParts)
        for (const part of data.spareParts) await useGarageStore.getState().addSparePart(part);
      if (data.invoices)
        for (const invoice of data.invoices) await useGarageStore.getState().addInvoice(invoice);
